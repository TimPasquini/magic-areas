...
from homeassistant.const import ATTR_ENTITY_ID, SERVICE_TURN_ON, SERVICE_TURN_OFF, STATE_OFF, STATE_ON
...
from tests.mocks import MockLight, MockBinarySensor
    ...
    data.update(
        {
            ...
            CONF_SECONDARY_STATES: {
                CONF_DARK_ENTITY: "binary_sensor.dark_sensor"
            }
        }
    )
    return MockConfigEntry(domain=DOMAIN, data=data)
...
async def setup_entities_binary_sensor_edge_cases(hass: HomeAssistant) -> list[MockBinarySensor]:
...
async def test_is_child_controllable(
    ...
    assert not target_group.is_child_controllable(entities_light_edge_cases[0].entity_id)
    
    # Test entity found AND has controlling attribute (True)
    hass.states.async_set(entities_light_edge_cases[0].entity_id, STATE_ON, {"controlling": True})
