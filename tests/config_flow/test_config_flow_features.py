...
async def test_options_flow_climate_no_presets(
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        user_input={MagicAreasFeatures.CLIMATE_CONTROL: True},
    )
...
async def test_options_flow_fan_groups(hass: HomeAssistant, init_integration: MockConfigEntry) -> None:
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        user_input={MagicAreasFeatures.FAN_GROUPS: True},
    )
...
async def test_options_flow_area_aware_media_player(
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        user_input={MagicAreasFeatures.AREA_AWARE_MEDIA_PLAYER: True},
    )
...
async def test_options_flow_aggregates(hass: HomeAssistant, init_integration: MockConfigEntry) -> None:
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        user_input={MagicAreasFeatures.AGGREGATES: True},
    )
...
async def test_options_flow_presence_hold(
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        user_input={MagicAreasFeatures.PRESENCE_HOLD: True},
    )
...
async def test_options_flow_ble_trackers(hass: HomeAssistant, init_integration: MockConfigEntry) -> None:
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        user_input={MagicAreasFeatures.BLE_TRACKER: True},
    )
...
async def test_options_flow_wasp_in_a_box(
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        user_input={MagicAreasFeatures.WASP_IN_A_BOX: True},
    )
...
async def test_options_flow_health(hass: HomeAssistant, init_integration: MockConfigEntry) -> None:
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        user_input={MagicAreasFeatures.HEALTH: True},
    )
...
async def test_options_flow_remove_feature(
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        user_input={MagicAreasFeatures.LIGHT_GROUPS: False},
    )
...
async def test_options_flow_add_feature(hass: HomeAssistant, init_integration: MockConfigEntry) -> None:
...
async def test_options_flow_feature_conf_updates_options(
    ...
    flow.context = cast(Any, {"step_id": "feature_conf_health"})
    result = await flow.async_step_feature_conf(
        user_input={CONF_HEALTH_SENSOR_DEVICE_CLASSES: ["problem"]}
    )

    assert result["type"] == FlowResultType.MENU
    assert MagicAreasFeatures.HEALTH in flow.area_options[CONF_ENABLED_FEATURES]
