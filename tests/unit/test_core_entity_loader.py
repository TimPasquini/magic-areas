...
from custom_components.magic_areas.core.entity_loader import (
    filter_entity_list,
...
def create_mock_entity(
    ...
    entity.disabled = disabled_by is not None
...
class TestFilterEntityList:
    ...
    def test_filter_entity_list_comprehensive(
        ...
        result = filter_entity_list(
            entity_list,
            "magic_config_entry",
            exclude_list=["sensor.pressure"],
            ignore_diagnostic=True,
        )
        # Only mock_entity should remain
        assert len(result) == 1
        assert result[0] == mock_entity
