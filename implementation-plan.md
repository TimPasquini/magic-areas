...
- update entity identity and availability handling to align with HA guidance
...
- align `unique_id` composition with HA guidance (durable IDs, no domain)
- implement coordinator-driven availability flags on entities
- refactor `AREA_STATE_CHANGED` to carry full current state snapshot and remove
  stale state checks in handlers (see reference plan below)

Phase 2 TODO (ordered):

- extract aggregates logic into `core/aggregates.py` and update `sensor/__init__.py`
  and `binary_sensor/__init__.py` to use core outputs
- finish entity list assembly split:
  - keep registry/state reads in `base/magic.py`
  - move grouping/filtering into `core/entities.py`
  - update coordinator snapshot build to consume core outputs
- reduce `base/magic.py` to orchestration only once core modules cover presence,
  aggregates, and entity grouping
- align remaining state lookups:
  - remove `self.area.has_state(...)` from handlers
  - use event payload `current_states` or a shared state helper
- expand tests:
  - add HA-free unit tests for `core/aggregates.py` and entity grouping
  - add coordinator snapshot integration tests across platforms
- verify unique_id + availability alignment across platforms

Reference: area state change event refactor plan

- update dispatcher payload to include `current_states`
- update all handlers to consume the event snapshot only:
  - `switch/climate_control.py`
  - `light.py`
  - `switch/fan_control.py`
  - `switch/media_player_control.py`
- remove `self.area.has_state(...)` usage inside event handlers
- align non-event code paths to use a single helper for current state lookup
- update tests that assert or construct event payloads
- add regression coverage for stale-state ordering (occupied vs clear)
